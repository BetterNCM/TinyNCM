...
function Checkbox({ name, value, onClick, color }) {
    ...
    return (React.createElement("label", { style: { color, display: "flex", marginTop: ".3em" } },
...
function MinifyEle() {
    ...
    for (let collection in ElementMap) {
        ...
        for (let name in ElementMap[collection]) {
            ...
            configsEle.push((React.createElement(Checkbox, { name: name, value: config[ElementMap[collection][name]], color: (!document.querySelector(ElementMap[collection][name])) && "#ffffff99", onClick: (e) => {
...
function ColorsEle() {
    ...
    React.useEffect(() => {
        !(async () => {
            ...
            ncmColors = (await (await fetch('orpheus://orpheus/style/res/less/default/css/skin.ls.css')).text()).match(colorCSSRegex).map(v => {
                ...
                p[_a = v.originColor] ?? (p[_a] = { selectors: [], originColor: v.originColor, comment: null });
                (_b = p[v.originColor]).comment ?? (_b.comment = v.comment);
    ...
    return (React.createElement("div", null,
        ...
        Object.values(colors).map(color => {
            ...
            return (React.createElement("div", null,
                React.createElement("span", { style: { backgroundColor: "white", color: "black" } },
                    (manualComment && "(√) " + manualComment) ?? color.comment ??
                        (chatgptComment && "ChatGPT: " + chatgptComment) ?? "未知",
                    " ",
                    varSelectorComment ? `${varSelectorComment}` : ''),
...
plugin.onConfig((tools) => {
    ...
    let settingsEle = (React.createElement("div", null,
        ...
        React.createElement(Expandable, { title: "\u989C\u8272\u4FEE\u6539" },
            React.createElement(ColorsEle, null))));
